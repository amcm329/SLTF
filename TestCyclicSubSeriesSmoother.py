...
class CyclicSubSeriesSmootherTest(unittest.TestCase):
      ...
      def test_TrendingSinusoidExtrapolationTest(self):
          ...
          extendedData = np.empty(4 * period)
          builder = CyclicSubSeriesSmoother.Builder().setWidth(7) #Sub-cycle data is linear so width shouldn't matter
          sssmoother = builder.setDataLength(len(data)).setPeriodicity(period).extrapolateForwardAndBack(1).build()
